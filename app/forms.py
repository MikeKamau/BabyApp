...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if user is not None:
            raise ValidationError('Please use a different username, that username already exists')

    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user is not None:
            raise ValidationError('Please use a different email, that one is already in use')
